...
function SalesReturns() {
    ...
    const permissions = user?.permissions?.sales_returns || {};
    ...
    const loadData = async () => {
        setLoading(true);
        try {
            const [returnsData, customersData, invoicesData, accountsData] = await Promise.all([
    ...
    const handleInvoiceChange = async (invoiceId) => {
        ...
        try {
            ...
            // Map items with sold quantity, returned quantity, and current return quantity
    ...
    const handleSubmit = async (e) => {
        ...
        if (!formData.customer_id) {
            setError(t('customer_required') || 'يجب اختيار العميل');
            return;
        }
    ...
    const filteredReturns = returns.filter(ret => {
        ...
        const matchesCustomer = !customerFilter || String(ret.customer_id) === customerFilter;
    ...
    return (
        <div>
            ...
            {/* Create Return Modal */}
            <Modal isOpen={showModal} onClose={closeModal} title={t('add_return') || 'إرجاع مبيعات'} size="lg">
