...
function SalesInvoices() {
    ...
    const printInvoice = async () => {
        ...
        const logoHtml = logoBase64 ? `<img src="${logoBase64}" alt="Logo" style="max-height:70px;max-width:160px;object-fit:contain" />` : '';
        const showCompanyInfo = settings.invoice?.show_company_info !== 'no';
        const companyInfoHtml = showCompanyInfo ? `<div style="line-height:1.6"><h1 style="margin:0;font-size:22px;color:#1a365d;font-weight:700">${companyName}</h1>${companyAddress ? `<p style="margin:2px 0;font-size:11px;color:#64748b">${companyAddress}</p>` : ''}${companyPhone ? `<p style="margin:2px 0;font-size:11px;color:#64748b">هاتف: ${companyPhone}</p>` : ''}${companyEmail ? `<p style="margin:2px 0;font-size:11px;color:#64748b">${companyEmail}</p>` : ''}${companyTaxNumber ? `<p style="margin:2px 0;font-size:11px;color:#64748b">الرقم الضريبي: ${companyTaxNumber}</p>` : ''}</div>` : '';
